...
class DatabaseHelper(context: Context?) :
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        db?.execSQL(SQL_CREATE_RESTAURANT_TYPES_TABLE_EXTRA_KEY)
        db?.execSQL(SQL_CREATE_LAST_MODIFICATION_TABLE_EXTRA_KEY)
        db?.execSQL(SQL_CREATE_CREATE_TRIGGER_ON_RESTAURANT_TYPES_TABLE_TABLE_EXTRA_KEY)
    }
    ...
    override fun insert(tableName: String, item: RestaurantEntity?): Boolean {
        val writableDatabase = writableDatabase
        val newRowId: Long?
        val contentValues = ContentValues().apply {
            put(COLUMN_NAME_RESTAURANT_ID_EXTRA_KEY, item?.id)
            put(COLUMN_NAME_TYPE_EXTRA_KEY, item?.type)
            ...
            put(COLUMN_NAME_TYPE_POPULARITY_EXTRA_KEY, item?.typePopularity)
    ...
    companion object {
        ...
        const val DATABASE_NAME = "RestaurantTypesDatabase.db"
    }
}